 ...
 * @type {ActionType}
 */
const validateFood = ctx => {
	if (
		ctx.accamulatedVeganFood < 0
		|| ctx.accamulatedFish < 0
		|| ctx.accamulatedGame < 0
	) {
		return DAY_RESULT.DEAD_END
	}
	return DAY_RESULT.NONE
}
/**
 * @type {{[key: string]: (ctx: Context) => void}} EAT_FOOD
 */
const EAT_FOOD = {
	VEGAN: ctx => {
		if (ctx.accamulatedVeganFood > 0) {
			ctx.accamulatedVeganFood -= 1
		}
		else {
			throw new Error('no vegan food to eat')
		}
	},
	FISH: ctx => {
		if (ctx.accamulatedFish > 0) {
			ctx.accamulatedFish -= 1
		}
		else {
			throw new Error('no fish to eat')
		}
	},
	GAME: ctx => {
		if (ctx.accamulatedGame > 0) {
			ctx.accamulatedGame -= 1
		}
		else {
			throw new Error('no game to eat')
		}
	},
}
